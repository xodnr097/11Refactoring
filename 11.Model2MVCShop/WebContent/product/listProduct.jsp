...
<form name="detailForm" action="<% if(request.getParameter("menu").equals("manage")){ %>
								/product/listProduct?menu=manage
								<% } else { %>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<c:forEach var="product" items="${list}">
		...
		<tr class="ct_list_pop">
			...
			<td align="left">
				<c:if test="${param.menu == 'manage' }">
				<a href="/product/getProduct?prodNo=${product.prodNo}&menu=manage">${product.prodNo}</a></td>
				</c:if>
				<c:if test="${param.menu == 'search' }">
				<a href="/product/getProduct?prodNo=${product.prodNo}&menu=search">${product.prodNo}</a></td>
				</c:if>
			</td>
			<td></td>
